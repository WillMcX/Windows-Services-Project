...
if (-not $env:RUNNING_IN_PWSH7 | Out-Null) {
    if ($PSVersionTable.PSEdition -ne 'Core' -or $PSVersionTable.Major -lt 7) {
        ...
        Start-Process -FilePath $pwshExePath -ArgumentList "-NoProfile", "-ExecutionPolicy Bypass", "-File `"$scriptPath`"" -NoNewWindow
        exit
    }
}

function Log-PowerShellVersion {
    param (
        [ref]$logBox
    )
    $psVersion = $PSVersionTable.PSVersion
    $psEdition = $PSVersionTable.PSEdition
    $message = "Running in PowerShell $psEdition $($psVersion.Major).$($psVersion.Minor).$($psVersion.Build)"
    
    Write-Host $message
    Write-Log -Message $message -TextBox $logBox
}
...
function Log-PowerShellVersion {
    ...
    [System.Windows.Forms.MessageBox]::Show($message, "PowerShell Version")

    if ($logBox -ne $null) {
        Write-Log -Message $message -TextBox ([ref]$logBox)
    }
}
