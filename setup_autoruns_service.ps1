...
$serviceName = "AutorunsPeriodicScannerTestv7.1"
...
# Function to update or edit API key
function Edit-ApiKey {
    if (Test-Path $apiKeyPath) {
        Write-Host "Current API key found: "
        $currentApiKey = Get-Content -Path $apiKeyPath -Raw
        Write-Host $currentApiKey
        $updateKey = Read-Host "Do you want to update the API key? (yes/no)"
        if ($updateKey -eq "yes") {
            $newApiKey = Read-Host "Please enter the new VirusTotal API key"
            Set-Content -Path $apiKeyPath -Value $newApiKey
            Write-Host "API key updated."
        } else {
            Write-Host "API key not changed."
        }
    } else {
        Write-Host "No API key found. Please enter a new one."
        $newApiKey = Read-Host "Please enter the VirusTotal API key"
        Set-Content -Path $apiKeyPath -Value $newApiKey
...
$userInput = Read-Host "What would you like to do? (stop/restart/install/uninstall/edit-api/exit)"
