...
$whitelistFilePath = "$downloadFolder\whitelist.csv" 
...
function Load-Whitelist {
    ...
    if (Test-Path $whitelistFilePath) {
        try {
            ...
            Write-Host "Error reading whitelist CSV file: $_"
        }
    } else {
        Write-Host "Whitelist file not found. Creating an empty CSV whitelist."
